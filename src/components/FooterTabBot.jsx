...
import { footerTabBotLinks } from '../constants'
const FooterTabBot = () => {
  return (
    <div className='flex-[1.5] w-full flex flex-row justify-between md:mt-0  border-b-2 pb-1 border-textMain/10  lg:hidden'>
    {footerTabBotLinks.map((footerTabBotLink, titleIndex) => (
      <div key={footerTabBotLink.key} className={`flex flex-col ss:my-0 my-4 px-8 xs:px-0 min-w-[80px] w-full`}>
        <h4 className='text-textMain cursor-default'>
          {footerTabBotLink.title}
        </h4>
        <ul className='grid grid-cols-2'>
          {footerTabBotLink.links.map((link, linkIndex) => {
            return (
